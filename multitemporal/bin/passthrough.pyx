...
def passthrough(np.ndarray[np.float32_t, ndim=3, negative_indices=False] data not None,
                float missingval,
    ...
    cdef float nothing = params[0]

    cdef int nin = get_nout(nfr, params)
    cdef np.ndarray[np.float32_t, ndim=3] result = np.zeros((nin,nyr,npx), dtype='float32')
