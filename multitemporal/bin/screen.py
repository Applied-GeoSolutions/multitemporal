...
def screen(np.ndarray[np.float32_t, ndim=3, negative_indices=False] data not None,
    ...
    cdef unsigned int nfr = data.shape[1]
    cdef unsigned int nyr = data.shape[2]
    cdef unsigned long npx = data.shape[3]
    ...
    for k in range(npx):
        for j in range(nyr):
            for i in range(nfr):
                ...
                if data[0,i,j,k] != missingval:
                    if data[0,i,j,k] < min1 or data[0,i,j,k] > max1:
